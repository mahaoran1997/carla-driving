...
def step_subsection(k, global_step_op):
  with tf.name_scope('subsection'):
    
    st = k/(k + math.exp(2000.0/k))
    ed = k/(k + math.exp(3000.0/k))
    grad = (ed-st)/6000.0
    print(st, ed, grad)
    tmp_g = tf.constant(grad, dtype=tf.float32) * (tf.cast(global_step_op, tf.float32) - tf.constant(2000.0, dtype=tf.float32))
    tmp_g = tmp_g + tf.constant(st, dtype=tf.float32)
    with tf.name_scope('inverse_sigmoid_decay_in'):

      print("---------------------important------------")

      print(global_step_op)
      k = tf.constant(k, dtype=tf.float32) #mhr:!!!! no /10
      tmp1 = k*tf.exp(-tf.cast(global_step_op, tf.float32)/k)
      tmp1 = tmp1 / (1. + tmp1)
      #print(g_step)
      g_step = global_step_op - tf.constant(5000, dtype=tf.int64)
      tmp2 = k*tf.exp(-tf.cast(g_step, tf.float32)/k)
      tmp2 = tmp2 / (1. + tmp2)

    out_pre = tf.cond(tf.less(global_step_op, 2000), lambda: tmp1, lambda: tmp_g)
    out = tf.cond(tf.less(global_step_op, 8000), lambda: out_pre, lambda: tmp2)
  return out
...
def concat_state_w(f, names):
  af = {}
  for k in names:
    af[k] = np.concatenate([x[k] for x in f], axis=0)
    # af[k] = np.swapaxes(af[k], 0, 1)
  return af
...
def sample_action(rng, action_probs, optimal_action, sample_gt_prob,
  ...
  #action, action_sample_wt = sample_action(rng_action, action_probs, optimal_action, sample_gt_prob,
  ...
  for i in range(batch_size):
    ...
    if combine_type == 'one_or_other':
      if sample_gt and type == 'sample': 
        action.append(optimal_action[i])
        action_sample_wt[i] = sample_gt_prob
        #distr_ = optimal_action_[i,:]*1.
      else: 
        action.append(action_probs[i])
        action_sample_wt[i] = 1-sample_gt_prob
...
def train_step_custom_online_sampling(sess, train_op, global_step,
  ...
  all_state_targets = concat_state_w(state_targets, e.get_targets_name())
  all_state_features = concat_state_w(state_features,
                                      e.get_features_name())
  all_step_data_cache = concat_state_w(step_data_cache,
                                        m.train_ops['step_data_cache'])
...
def test_step_custom_online_sampling(sess, train_op, global_step,
  ...
  while iter_id < iters:
    ...
    action, action_sample_wt = sample_action(
        rng_action, action_probs, optimal_action, sample_gt_prob,
        m.sample_action_type, m.sample_action_combine_type)
    rst = e.take_action(action, net_state)
    if rst[0]:
      iter_id += 1
    j+=1
