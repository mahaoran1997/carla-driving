    ...
    with make_carla_client(drive_config.host, drive_config.port) as client:
        ...
        while direction != -1:
            ...
            if (time_iteration > drive_config.reset_period) \
                    or (
                    _latest_measurements.player_measurements.collision_vehicles > 0.0 or _latest_measurements.player_measurements.collision_pedestrians > 0.0 or _latest_measurements.player_measurements.collision_other > 0.0):
                _start_time = time.time()  # ???
                time_iteration = 0
                # print (self._config_path)
            ...
            # print actions
            if recording:
                recorder.record(measurements, sensor_data, actions, action_noisy,
                                direction)  # , driver.get_waypoints())
