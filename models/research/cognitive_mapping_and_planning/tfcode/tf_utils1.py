...
def train_step_custom_online_sampling(sess, train_op, global_step,
  ...
  for i in range(iters):
    ...
    #print ('success!')
    #output = open('tmp_files/common_data.txt','w')
    #output.write(input)
    #output.close()
    ...
    executed_actions = np.array(executed_actions).transpose((1,0,2)) #.T
...
'''for o_map_array in input['orig_maps']:
  print ('len(o_map_array)')
  print (len(o_map_array))
  o_map = o_map_array[0]
  print ('len(o_map)')
  print (len(o_map))
