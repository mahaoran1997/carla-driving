r"""Carla Environment. Includes the following classes along with some
helper functions.
...
class CarlaEnvWrapper:
    ...
    def get_instance(self):
        # TODO: self.carla_config needs to chosen randomly
        self.carla_config = './datasets/inuse/drive_interfaces/carla/CarlaSettings3CamTest.ini'
        self.drive_config.carla_config = self.carla_config
        #print self.drive_config
        driver = CarlaEnv(self.drive_config)

        # carla_config_id = random.ch
        self.camera_dict = get_camera_dict(self.drive_config.carla_config)
        print " Camera Dict "
...
class CarlaEnv(Driver):
    ...
    def _reset(self):
        ...
        self.episode_config = find_valid_episode_position(
            self.positions, self._agent.waypointer)
    ...
    def get_sensor_data(self, goal_pos=None, goal_ori=None):
        ...
        return measurements, direction, reach_goal, action_noisy
    ...
    def get_data_for_mapper(self, typ='rgb'):
        ...
        measurements, direction, reach_goal, action_noisy = self.get_sensor_data()
        capture_time = time.time()
        if typ == 'rgb':
            image = measurements['BGRA'][0][self._driver_conf.image_cut[0]:self._driver_conf.image_cut[1], :, :3]
            image = image[:, :, ::-1]
            image = scipy.misc.imresize(image, [self._driver_conf.image_size2, self._driver_conf.image_size1])
            Image.fromarray(image).save("datasets/inuse/cog/img_" + str((capture_time)) + ".png")
        elif typ == 'd':
            image = measurements['Depth'][0][self._driver_conf.image_cut[0]:self._driver_conf.image_cut[1], :, :3]
            image = scipy.misc.imresize(image, [self._driver_conf.image_size2, self._driver_conf.image_size1])
            Image.fromarray(image).save("datasets/inuse/cog/dep_" + str((capture_time)) + ".png")
        else:
            logging.fatal('Sampling not one of uniform.')
        return image, measurements, direction, reach_goal, action_noisy
